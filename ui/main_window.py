...
from utils.file_utils import FileRecord, collect_video_files, human_size, probe_file_record
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.table.setHorizontalHeaderLabels(["Filename", "Duration", "Resolution", "Size"])
    ...
    def add_paths(self, raw_paths: list[str]) -> None:
        try:
            files = collect_video_files([Path(path) for path in raw_paths])
        except ValueError as exc:
            QMessageBox.warning(self, "Input problem", str(exc))
            return

        if not files:
            QMessageBox.information(self, "No videos found", "No supported video files were found.")
            return
    ...
    def start_processing(self) -> None:
        if not self.records:
            QMessageBox.information(self, "No files", "Drop or choose at least one video file first.")
            return
        ...
        self.worker = BatchProcessor(self.records, options, self.runner)
